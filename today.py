...
def recursive_loc(owner, repo_name, data, cache_comment, addition_total=0, deletion_total=0, my_commits=0, cursor=None, retries=3):
    ...
    if request.status_code in [500, 502, 504] and retries > 0:
        time.sleep(2)
...
if __name__ == '__main__':
    ...
    # Calculate account creation date for uptime
    account_creation = datetime.datetime.strptime(acc_date, "%Y-%m-%dT%H:%M:%SZ")
    age_data, age_time = perf_counter(daily_readme, account_creation)
